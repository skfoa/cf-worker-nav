...
- 🕵️ **隐私级图标代理** — 独创的四级降级图标代理机制（DuckDuckGo → favicon.im → 直连网站 → 首字母 SVG 生成），所有请求均由 Worker 服务端发起，彻底切断第三方追踪器对您私有书签域名的隐私窥探，确保图标 100% 显示。
...
**Q: 图标显示异常怎么办？**
> 图标通过四级降级代理获取（DuckDuckGo → favicon.im → 直连网站 → 自动生成首字母 SVG），所有请求均由 Worker 服务端发起，不会暴露用户隐私。如果某个图标显示异常，可在编辑链接时点击「🔄 刷新图标缓存」手动清除边缘缓存。也可以在图标 URL 字段手动填写高质量图标地址（如网站的 SVG logo）来覆盖自动获取。
